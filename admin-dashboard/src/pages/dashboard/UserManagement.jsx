...
import { FaSearch, FaFilter, FaFileExport, FaPlus, FaUserEdit, FaTrash } from "react-icons/fa"
...
const UserManagement = () => {
  ...
  const form = useForm({
    defaultValues: {
      ...
      email: "",
      phone: "",
      role: "merchant", // Changed from "user" to "merchant"
      isActive: true,
    },
  })
  ...
  const viewUserDetails = (user) => {
    ...
    form.reset({
      ...
      email: user.email || "",
      phone: user.phone || "",
      role: user.role || "merchant", // Changed from "user" to "merchant"
  ...
  // Updated to only send the role field for updates
  const handleUpdateUser = async (data) => {
    try {
      setLoading(true)
      // Only send role for updates since other fields are rejected
      const updateData = { role: data.role }
      const response = await apiService.patch(`/users/${selectedUser._id}`, updateData)

      if (response.status === "success") {
        const updatedUsers = users.map((user) => (user._id === selectedUser._id ? { ...user, ...updateData } : user))
  ...
  // Updated to send only the role field for user creation
  const handleCreateUser = async (data) => {
    try {
      setLoading(true)
      // Only send role for creation since other fields are rejected
      const createData = { role: data.role }
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className="space-y-6">
      ...
      <div className="rounded-md border border-gray-200 dark:border-gray-700">
        {loading ? (
          ...
          <div className="text-center py-10 text-gray-500">No users found matching your criteria</div>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
              ...
              <form onSubmit={form.handleSubmit(handleUpdateUser)} className="space-y-6">
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            {...field}
                            disabled={true} // Always disabled since API doesn't accept this field
                    ...
                    render={({ field }) => (
                      <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                        <div className="space-y-0.5">
                          ...
                          <FormDescription>{field.value ? "User is verified" : "User is not verified"}</FormDescription>
                        </div>
                        <FormControl>
                          <Switch
                            checked={field.value}
                ...
                <DialogFooter className="gap-2">
                  {isEditMode ? (
                    ...
                    <>
                      <Button type="button" variant="outline" onClick={() => setUserDetailsOpen(false)}>
                        Close
                      </Button>
                      <Button
                        type="button"
      ...
      <Dialog open={createUserOpen} onOpenChange={setCreateUserOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>Add a new user to the system. Only role can be set during creation.</DialogDescription>
          </DialogHeader>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>This field cannot be set via API</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                />
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Email Address</FormLabel>
                      <FormControl>
                        <Input {...field} placeholder="Email Address" type="email" disabled />
                      </FormControl>
                      <FormDescription>This field cannot be set via API</FormDescription>
                      <FormMessage />
                    </FormItem>
      ...
      <Dialog open={deleteConfirmOpen} onOpenChange={setDeleteConfirmOpen}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter className="gap-2">
            ...
            <Button type="button" className="bg-red-600 hover:bg-red-700" onClick={handleDeleteUser}>
              <FaTrash className="mr-2" /> Delete User
            </Button>
          </DialogFooter>
        </DialogContent>
