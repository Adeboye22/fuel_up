...
const NavBar = () => {
  ...
  // this function toggles the services link for more options in mobile mode
  const toggleClick = () => {
    setIsOpen(!isOpen)
  }
    ...
    <div className='flex flex-row justify-between bg-white mt-4'>
        ...
        <div className={'sm:fixed sm:left-0 sm:top-4 sm:border-0 sm:rounded-tr-xl sm:bg-lime sm:h-full sm:w-3/4 sm:text-center sm:p-16 text-lg -my-4'}>
            <ul className='sm:flex sm:flex-col sm:gap-4 sm:my-36 sm:text-white sm:text-lg text-gray flex flex-row gap-8 px-8 font-medium'>
              <li className='hover:text-lime'><NavLink to="/" onClick={closeNavBar} style={activeNav}>Home</NavLink></li>
              <li className='hover:text-lime'><NavLink to="services" onClick={closeNavBar} style={activeNav}>Services</NavLink></li>
              <li className='hover:text-lime'><NavLink to="pricing" onClick={closeNavBar} style={activeNav}>Pricing</NavLink></li>
              ...
              <li className='hover:text-lime'><NavLink to="signup" onClick={closeNavBar} style={activeNav}>Sign up</NavLink></li>
            </ul>
        </div>
    </div>
  )   
